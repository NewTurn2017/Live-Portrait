...
def get_safe_filename(filename):
    """Convert a filename to a safe version that works on both Windows and Linux."""
    # Remove or replace characters that are problematic in either OS
    invalid_chars = '<>:"/\\|?*'
    for char in invalid_chars:
        filename = filename.replace(char, '_')
    # Ensure the filename doesn't start or end with a space or period
    filename = filename.strip('. ')
    # Limit the length of the filename (Windows has a 255 character limit)
    return filename[:240]  # Leave some room for the numbering


def get_incremental_filename(base_path, extension):
    counter = 1
    while True:
        new_path = f"{base_path}_{counter:04d}{extension}"
...
class LivePortraitPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if is_video(args.driving_info):
            ...
            inference_cfg.flag_eye_retargeting = args.flag_eye_lip_open_enabled
            inference_cfg.flag_lip_retargeting = args.flag_eye_lip_open_enabled
            if inference_cfg.flag_eye_retargeting or inference_cfg.flag_lip_retargeting:
                ...
                input_eye_ratio_lst, input_lip_ratio_lst = self.live_portrait_wrapper.calc_retargeting_ratio(
                    source_lmk, 
                    driving_lmk_lst, 
                    args.target_eye_ratio, 
                    args.target_lip_ratio
                )
        ...
        # Use source image name for output files
        source_name = get_safe_filename(osp.splitext(osp.basename(args.source_image))[0])
        ...
        if is_video(args.driving_info):
            ...
            base_concat = osp.join(args.output_dir, f'{source_name}_concat')
            wfp_concat = get_incremental_filename(base_concat, '.mp4')
            images2video(frames_concatenated, wfp=wfp_concat)
            base_concat_sound = osp.join(args.output_dir, f'{source_name}_concat_sound')
            wfp_concat_sound = get_incremental_filename(base_concat_sound, '.mp4')
            add_audio_to_video(wfp_concat, args.driving_info, wfp_concat_sound)
            os.remove(wfp_concat)
    
        # save drived result
        base_output = osp.join(args.output_dir, f'{source_name}')
        wfp = get_incremental_filename(base_output, '.mp4')
        if inference_cfg.flag_pasteback:
            images2video(I_p_paste_lst, wfp=wfp)
        else:
            images2video(I_p_lst, wfp=wfp)
        base_output_sound = osp.join(args.output_dir, f'{source_name}_sound')
        wfp_sound = get_incremental_filename(base_output_sound, '.mp4')
        add_audio_to_video(wfp, args.driving_info, wfp_sound)
        os.remove(wfp)
        return wfp_sound, wfp_concat_sound