...
class LivePortraitWrapper(object):
    ...
    def calc_retargeting_ratio(self, source_lmk, driving_lmk_lst, target_eye_ratio, target_lip_ratio):
        input_eye_ratio_lst = []
        input_lip_ratio_lst = []
        for lmk in driving_lmk_lst:
            # Calculate original ratios
            orig_eye_ratio = calc_eye_close_ratio(lmk[None])
            orig_lip_ratio = calc_lip_close_ratio(lmk[None])
        
            # Adjust ratios based on target ratios
            adjusted_eye_ratio = orig_eye_ratio * ((target_eye_ratio * 100 + 100) / 100)
            adjusted_lip_ratio = orig_lip_ratio * ((target_lip_ratio * 100 + 100) / 100)
        
            input_eye_ratio_lst.append(adjusted_eye_ratio)
            input_lip_ratio_lst.append(adjusted_lip_ratio)
